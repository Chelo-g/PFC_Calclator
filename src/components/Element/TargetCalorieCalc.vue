<template>
  <div>
    <div>
        目標摂取カロリーは{{ this.$store.getters.TargetCalorie | unit_kCal }}です。
    </div>
    <div>
      <table class="calTable">
        <tr>
          <th>現在の体重(kg)</th>
          <th><input v-model.number="BodyWeight" type="number" @change="doTargetUpdata" min="0" value="aa"></th>
        </tr>
        <tr>
          <th>蛋白質(kcal)</th>
          <th><input v-model.number="ProteinCal" type="number" name="ProtainCalBox" @change="doProteinUpdata" min="0"></th>
        </tr>
        <tr>
          <th>脂質(kcal)</th>
          <th><input v-model.number="FatCal" name="FatCalBox" type="number"  @change="doFatUpdata" min="0"></th>
        </tr>
        <tr>
          <th>炭水化物(kcal)</th>
          <th><input v-model.number="CarbonateCal" type="number" name="CarbonateCalBox" @change="doCarbonateUpdata" min="0"></th>
        </tr>
        </table>
    </div>
    <div>
    ...
    <div>
      <el-progress type="circle" :percentage="parcentProtein" color="#F56C6C"></el-progress>
      <el-progress type="circle" :percentage="parcentFat" color="#E6A23C"></el-progress>
      <el-progress type="circle" :percentage="parcentCarbo" color="#67C23A"></el-progress>
    </div>
  </div>
</template>
...
<script>
import PFCBlanceChart from './PFCBlanceChart.vue'
// ストレージに保存
...
//        CarbonateCal:0
    ...
    computed:{
      watchCal(){
        var item ={TargetCalorie: this.$store.getters.TargetCalorie,ProteinCalorie: this.$store.getters.ProteinCalorie,FatCalorie: this.$store.getters.FatCalorie,CarbonateCalorie: this.$store.getters.CarbonateCalorie,BodyWeight: this.$store.getters.BodyWeight}
        return item;
      },
      parcentProtein(){
        return (this.$store.getters.ProteinCalorie/this.$store.getters.TargetCalorie * 100).toFixed(1)
      },
      parcentFat(){
        return (this.$store.getters.FatCalorie/this.$store.getters.TargetCalorie * 100).toFixed(1)
      },
      parcentCarbo(){
        return (this.$store.getters.CarbonateCalorie/this.$store.getters.TargetCalorie * 100).toFixed(1)
      },
    },
       methods: {
        ketoAddClick:function(){
            ...
            this.ProteinCal = Math.round(this.$store.getters.TargetCalorie -this.FatCal -this.CarbonateCal);
            //Vuexに渡す
        ...
        lowfatAddClick:function(){
            ...
            this.CarbonateCal = Math.round(this.$store.getters.TargetCalorie - this.FatCal - this.ProteinCal);
            //Vuexに渡す
            this.$store.dispatch("doProteinUpdata",this.ProteinCal * 1);
            this.$store.dispatch("doFatUpdata",this.FatCal* 1);
            this.$store.dispatch("doCarbonateUpdata",this.CarbonateCal* 1);
