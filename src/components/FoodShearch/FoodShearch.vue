<template>
  <div>
    <div>
      <table class="table">
        ...
        <tr>
          <th>最大値</th>
          <th>
            <input v-model.number="maxProtein" type="number" min="0" max="100" />
          </th>
          <th>
            <input v-model.number="maxFat" type="number" min="0" max="100" />
          </th>
          <th>
            <input v-model.number="maxCarbonate" type="number" min="0" max="100" />
          </th>
        </tr>
        <tr>
          <th>最小値</th>
          <th>
            <input v-model.number="minProtein" type="number" min="0" max="100" />
          </th>
          <th>
            <input v-model.number="minFat" type="number" min="0" max="100" />
          </th>
          <th>
            <input v-model.number="minCarbonate" type="number" min="0" max="100" />
          </th>
        </tr>
      </table>
    ...
    <table class="table">
      <thead>
        <tr>
          <th @click="sortBy('name')" :class="sortedClass('name')" class="thName">
            名前
          </th>
          <th @click="sortBy('Protein')" :class="sortedClass('Protein')" class="thPFC">
            蛋白質
          </th>
          <th @click="sortBy('Fat')" :class="sortedClass('Fat')" class="thPFC">
            脂質
          </th>
          <th @click="sortBy('Carbo')" :class="sortedClass('Carbo')" class="thPFC">
            炭水化物
          </th>
          <th></th>
        </tr>
      ...
      <tbody>
        <tr>
          <th>
            <input ref="addName" type="text" class="textbox" placeholder="自分で登録" />
          </th>
          <th>
            <input ref="addProtein" type="number" min="0" max="100" value="0" />
          </th>
          <th>
            <input ref="addFat" type="number" min="0" max="100" value="0" />
          </th>
          <th>
            <input ref="addCarbo" type="number" min="0" max="100" value="0" />
          </th>
...
export default {
    ...
    matched: function() {
      return this.list.filter(function(el) {
        return (
          this.minFat <= el.Fat &&
          el.Fat <= this.maxFat &&
          this.minProtein <= el.Protein &&
          el.Protein <= this.maxProtein &&
          this.minCarbonate <= el.Carbo &&
          el.Carbo <= this.maxCarbonate
        );
      }, this);
    },
    //表示件数調整
  ...
  methods: {
    ...
    addFoods() {
      ...
      var item = {
        weight: 100,
        Protein: parseFloat(addProtein.value),
        Fat: parseFloat(addFat.value),
        Carbo: parseFloat(addCarbo.value),
        name: addName.value,
