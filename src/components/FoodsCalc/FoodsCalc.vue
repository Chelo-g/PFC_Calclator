  ...
  <div>
    <div>
      <table class="calcTable">
        ...
        <tr>
          <th>合計値</th>
          <th>{{ (this.$store.getters.getsumParam('Protein') * 4).toFixed(1) }}</th>
          <th>{{ (this.$store.getters.getsumParam('Fat') * 9).toFixed(1) }}</th>
          <th>{{ (this.$store.getters.getsumParam('Carbo') * 4).toFixed(1) }}</th>
          <th>{{ totalCal }}</th>
        </tr>
        <tr>
          <th>目標-合計</th>
          <th
            :class="
              judgeRedText(
                this.$store.getters.ProteinCalorie - this.$store.getters.getsumParam('Protein') * 4
              )
            "
          >
            {{
          ...
          <th
            :class="
              judgeRedText(
                this.$store.getters.FatCalorie - this.$store.getters.getsumParam('Fat') * 9
              )
            "
          >
            {{
              (this.$store.getters.FatCalorie - this.$store.getters.getsumParam('Fat') * 9).toFixed(
                1
              )
            }}
          </th>
          <th
            :class="
              judgeRedText(
                this.$store.getters.CarbonateCalorie - this.$store.getters.getsumParam('Carbo') * 4
              )
            "
          >
            {{
          ...
          <th :class="judgeRedText(this.$store.getters.TargetCalorie - totalCal)">
            {{ (this.$store.getters.TargetCalorie - totalCal).toFixed(1) }}
          </th>
        </tr>
      </table>
    </div>
      ...
      <table class="listTable">
        ...
        <tbody>
          <tr v-for="(item, index) in sortList" :key="index">
            ...
            <th>
              <input type="number" v-model.number="item.weight" @change="doChartUpdate" step="10" />
            </th>
